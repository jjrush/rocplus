...
# @TEST-EXEC: zeek -C -r ${TRACES}/all_opcodes.pcap ${PACKAGE} %INPUT
...
# @TEST-EXEC: zeek-cut -n uid roc_plus_link_id < roc_plus_unknown_data.log > log.tmp && mv log.tmp roc_plus_unknown_data.log
# @TEST-EXEC: zeek-cut -n uid roc_plus_link_id < roc_plus_history_point_data.log > log.tmp && mv log.tmp roc_plus_history_point_data.log
# @TEST-EXEC: zeek-cut -n uid roc_plus_link_id < roc_plus_history_information.log > log.tmp && mv log.tmp roc_plus_history_information.log
# @TEST-EXEC: zeek-cut -n uid roc_plus_link_id < roc_plus_time_period_history_points.log > log.tmp && mv log.tmp roc_plus_time_period_history_points.log
# @TEST-EXEC: zeek-cut -n uid roc_plus_link_id < roc_plus_peer_to_peer_network_messages.log > log.tmp && mv log.tmp roc_plus_peer_to_peer_network_messages.log
